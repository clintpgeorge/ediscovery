...
from lucenesearch.lucene_index_dir import boolean_search_lucene_index, \
    get_indexed_file_details
...
def load_lda_parameters(mdl_cfg):
    ...
    print 'LDA-theta is loaded: # of documents:', num_docs, '# of topics:', num_topics, 'min(Theta):', min_lda_theta  
    ...
    print 'LDA-beta is loaded: # of topics:', num_topics, '# of terms in the vocabulary:', vocab_size, 'min(Bheta):', min_lda_beta
    print     
...
def eval_keywordlda_topiclda_lucene(file_prefix, config_file, 
    ...
    if valid_tokens  == 0:
        ...
        exit()
#
#    query_vec = lda_dictionary.doc2bow(tm_query.split())
...
#    
    ...
    query_td2 = get_lda_query_td2(tm_query, lda_dictionary, lda_beta)
    ...
    query_td = get_lda_query_td(tm_query, lda_dictionary, lda_mdl) 
...
def eval_ranking_varying_topics(query_id, dir_path, 
    ...
    for idx, num_topics in enumerate(topics): 
        ...
        config_file = "%sQ%d-LW-%dT.cfg" % (dir_path, query_id, num_topics)  # configuration file, created using the SMARTeR GUI 
        mdl_cfg = read_config(config_file)
